...
wr=tf.get_variable("wr", [n_relation, embed_dim],initializer=tf.random_uniform_initializer(minval=-bound,maxval=bound,seed=347))
trainable.append(wr)                                                                                                      	
...
input_wr_pos=tf.reshape(tf.nn.embedding_lookup(wr,train_input_pos[:,2]),[-1,embed_dim,1])
...
h_=input_h_pos-tf.matmul(input_wr_pos,input_h_pos,transpose_a=True)*input_wr_pos
t_=input_t_pos-tf.matmul(input_wr_pos,input_t_pos,transpose_a=True)*input_wr_pos
score_hrt_pos1=tf.norm(h_+input_r_pos-t_,ord=2,axis=1)
#L1
...
input_wr_neg=tf.reshape(tf.nn.embedding_lookup(wr,train_input_neg[:,2]),[-1,embed_dim,1])
...
h_=input_h_neg-tf.matmul(input_wr_neg,input_h_neg,transpose_a=True)*input_wr_neg
t_=input_t_neg-tf.matmul(input_wr_neg,input_t_neg,transpose_a=True)*input_wr_neg
score_hrt_neg1=tf.norm(h_+input_r_neg-t_,ord=2,axis=1)
#L1
