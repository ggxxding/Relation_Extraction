...
Eht_pos=tf.concat([input_h_pos,input_t_pos],axis=2)

eeee =tf.get_variable("eeeent_embedding", [2, 2],
                                                   initializer=tf.random_uniform_initializer(minval=-bound, \
                                                   	maxval=bound,seed=345))
inv2=tf.matrix_inverse(eeee)

EhtTEht=tf.matmul(tf.transpose(Eht_pos,perm=[0,2,1]),Eht_pos)
inv=tf.matrix_inverse(EhtTEht)
Ehtinv=tf.matmul(Eht_pos,EhtTEht)
Pht_pos=tf.matmul(Ehtinv,tf.transpose(Eht_pos,perm=[0,2,1]))


score_hrt_pos1=tf.norm(input_h_pos+tf.matmul(Pht_pos,input_r_pos) -input_t_pos,ord=2,axis=1)
#L1
...
Eht_neg=tf.concat([input_h_neg,input_t_neg],axis=2)
Pht_neg=tf.matmul(tf.matmul(Eht_neg,(tf.matmul(tf.transpose(Eht_neg,perm=[0,2,1]),Eht_neg))),tf.transpose(Eht_neg,perm=[0,2,1]))


score_hrt_neg1=tf.norm(input_h_neg+tf.matmul(Pht_neg,input_r_neg) -input_t_neg,ord=2,axis=1)
#L1
...
loss2=loss+tf.reduce_sum(tf.square(tf.norm(tf.reshape(tf.concat([input_h_pos,input_t_pos,input_h_neg,input_t_neg],axis=1),[-1,embed_dim]),axis=1))-1)+\
	  0.01* tf.reduce_sum(tf.square(tf.norm(tf.reshape(tf.concat([input_r_pos,input_r_neg],axis=1),[-1,embed_dim]),axis=1)))
#loss=tf.reduce_sum(tf.nn.relu(margin_+score_hrt_pos1-score_hrt_neg1))
'''+weight_*\
	...
	if is_train==1:
		...
		while n_epoch<num_epoch*2:
			...
			while(n_idx<n_triple):
				...
				#print(input_neg)
				a,loss_iter,_=sess.run([EhtTEht,loss2,op_train],\
