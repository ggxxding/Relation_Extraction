...
wr=tf.get_variable("wr", [n_relation, embed_dim*embed_dim],initializer=tf.random_uniform_initializer(minval=-bound,maxval=bound,seed=347))
trainable.append(wr)                                                                                                      	
...
input_wr_pos=tf.reshape(tf.nn.embedding_lookup(wr,train_input_pos[:,2]),[-1,embed_dim,embed_dim])
...
h_=tf.matmul(input_h_pos,input_wr_pos,transpose_a=True)
t_=tf.matmul(input_t_pos,input_wr_pos,transpose_a=True)
...
input_wr_neg=tf.reshape(tf.nn.embedding_lookup(wr,train_input_neg[:,2]),[-1,embed_dim,embed_dim])
...
h_=tf.matmul(input_h_neg,input_wr_neg,transpose_a=True)
t_=tf.matmul(input_t_neg,input_wr_neg,transpose_a=True)
