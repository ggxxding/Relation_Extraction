...
score_hrt_pos=tf.reduce_sum((input_h_pos+input_r_pos)*diag_pos*input_t_pos,axis=1)+\
tf.reduce_sum(input_h_pos*diag_pos*(input_t_pos-input_r_pos),axis=1)
...
#L1
score_hrt_neg=tf.reduce_sum((input_h_neg+input_r_neg)*diag_neg*input_t_neg,axis=1)+\
tf.reduce_sum(input_h_neg*diag_neg*(input_t_neg-input_r_neg),axis=1)
...
weight_diag_*(tf.norm(diag_pos,ord=2)+tf.norm(diag_neg,ord=2))
		...
		while n_epoch<num_epoch*2:
			...
			while(n_idx<n_triple):
				...
				hp,tp,rp,hn,tn,rn,ez,rz,loss_iter,_,diag_pos1=sess.run([hpn,tpn,\
					rpn,hnn,tnn,rnn,eZeroNorm,rZeroNorm,loss,op_train,diag_pos],{train_input_pos:input_pos,train_input_neg:input_neg})
				loss_sum+=loss_iter
